...
def set_locators(xvalues: list, ax: Axes = None, rotation: int=0):
    if isinstance(xvalues[0], datetime):
        ...
        if rotation:
            ax.set_xticklabels(xvalues, rotation=rotation, fontsize='small', ha='center')
        else:
            ax.set_xticklabels(xvalues, rotation=rotation,fontsize='small', ha='center')
        return None
    else:
...
def bar_chart(xvalues: list, yvalues: list, ax: Axes = None, title: str = '', xlabel: str = '', ylabel: str = '', percentage: bool = False, rotation: int = 0):
    ax = set_elements(ax=ax, title=title, xlabel=xlabel, ylabel=ylabel, percentage=percentage)
    #set_locators(xvalues, ax=ax, rotation=rotation)
    tick_vals = True
    if len(xvalues) > 25:
...
def plot_classification_report(classification_report, ax,title='Classification report ', cmap="viridis"):
    ax.set_title(title)
    sns.heatmap(pd.DataFrame(classification_report).iloc[:-1, :].T, annot=True,ax=ax,cmap=cmap,vmin=0.0,vmax=1.0)

def plot_evaluation_results(labels: ndarray, trn_y, prd_trn, tst_y, prd_tst,axBar1,axBar2,axConfusion,modelName):
    # cnf_mtx_trn = confusion_matrix(trn_y, prd_trn, labels=labels)
    cnf_mtx_tst = confusion_matrix(tst_y, prd_tst, labels=labels)
    plot_confusion_matrix(cnf_mtx_tst, labels, ax=axConfusion, title='Test')
    # print(classification_report(trn_y,prd_trn,labels=labels,output_dict=True))
    # print("\n")
    # print(classification_report(tst_y,prd_tst,labels=labels,output_dict=True))
    plot_classification_report(classification_report=classification_report(trn_y,prd_trn,labels=labels,output_dict=True),ax=axBar1,title=f"Classification report for {modelName} (training)")
    plot_classification_report(classification_report=classification_report(tst_y,prd_tst,labels=labels,output_dict=True),ax=axBar2,title=f"Classification report for {modelName} (test)")
...
def horizontal_bar_chart(elements: list, values: list, error: list = None, ax: Axes = None, title: str = '', xlabel: str = '', ylabel: str = ''):
